...
import Cookies from 'js-cookie'; // Import js-cookie library for easy cookie management
import CryptoJS from 'crypto-js'; // For basic password encryption in the cookie
...
const Login = () => {
  ...
  // Cookie encryption key - in a real app, use an environment variable
  const COOKIE_SECRET = "industrial-pro-app-secret-key";

  // Check for remembered credentials on component mount
  useEffect(() => {
    const savedEmail = Cookies.get('userEmail');
    const savedPasswordEncrypted = Cookies.get('userPassword');
    
    if (savedEmail && savedPasswordEncrypted) {
      try {
        // Decrypt the password
        const bytes = CryptoJS.AES.decrypt(savedPasswordEncrypted, COOKIE_SECRET);
        ...
        // If decryption fails, just use the email
        setFormData(prev => ({
          ...prev,
          email: savedEmail
        }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await postRequest("/auths/login", loginPayload);
      ...
      if (response.data) {
        ...
        // Store authentication data in localStorage (session storage)
        ...
        if (rememberMe) {
          ...
          Cookies.set('userPassword', encryptedPassword, { expires: 7 });
        } else {
  ...
  // Toggle remember me and handle cookies accordingly
  const handleRememberMeToggle = (e) => {
    const isChecked = e.target.checked;
    setRememberMe(isChecked);
    
    // If unchecked, immediately remove the cookies
    if (!isChecked) {
      Cookies.remove('userEmail');
      Cookies.remove('userPassword');
    }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        <div className="text-center">
          ...
          <p className="text-gray-600">Sign in to your IndustrialPro account</p>
        </div>
      </div>

      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="login-card bg-white py-8 px-4 shadow-xl sm:rounded-lg sm:px-10">
